...
namespace Pomelo.WebglPomelo
{
    public class Protocol
    {
        ...
        private WebglPomeloClient wpc;

        public WebglPomeloClient getPomeloClient()
        {
            return this.wpc;
        }

        public Protocol(WebglPomeloClient wpc, WebSocketJS.WebSocket socket)
        {
            this.wpc = wpc;
        ...
        {
            PackAndSend(route, 0, msg);
        }

        /// <summary>
        /// <para>打包然后发送</para>
        /// <para>route：例如：gate.gateHandler.queryEntry</para>
        /// </summary>
        /// <param name="route">例如：gate.gateHandler.queryEntry</param>
        internal void PackAndSend(string route, uint id, JsonObject msg)
        {
        ...
        internal void processMessage(byte[] bytes)
        {
            ...
            {
                this.heartBeatService.resetTimeout();
                wpc.processMessage(messageProtocol.decode(pkg.body));
            }
